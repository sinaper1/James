...
import {getStatisticsList, getTotalList} from "@/services/v1/StatisticsController";
...
const StatisticsPage: React.FC = () => {
    ...
    const getDataList = async () => {
        ...
        const {data, success} = await getTotalList({seasonType: state.seasonType});
        if(success){
            setState((prevState)=>({
                ...prevState,
                totalData: data,
        ...
        const {data: statisticsData, success: statisticsSuccess} = await getStatisticsList({
            type: state.type,
                    ...
                    >
                        <>
                            <div className={'statistics-total'}>
                                {
                                    state?.totalData && Object.keys(state?.totalData as API.TotalItem)?.map((item: string, index: number)=>(
